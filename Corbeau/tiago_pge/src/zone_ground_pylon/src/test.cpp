...
// Recuperation du points le plus proche sur le cercle
	...
	for (int i = 0; i < numOfPoints; i++){
		distance = pow(sqrt(points[0][i] - pointOrigine[0]), 2) + pow(sqrt(points[1][i] - pointOrigine[1]), 2);
		if(distance < bestPt){
			bestPt = distance;
			indice = i;
		}
	...
	bestPt = INFINITY;
	for (int i = 0; i < numOfPoints; i++){
		distance = pow(sqrt(points[0][i] - pointGoal[0]), 2) + pow(sqrt(points[1][i] - pointGoal[1]), 2);
		if(distance < bestPt){
			bestPt = distance;
			indice = i;
		}
	...
	std::cout << "Le point le plus proche de l'origine est : " << pointGoal[0] << " " << pointGoal[1] << " est : " << std::endl;
